...
let disabledperma = [
  "sharebot", "sharelist", "stoprent", "disableshare", 
  "enableshare", "setsudo", "delsudo", "newvar", "delvar", 
  "setvar", "update", "updatenow", "restart", "reboot"
];
...
db.update = async (nouvelleDonnee) => {
  try {
    const cheminFichier = "./asta.json";
    const ancienneDonnee = db.get();
    const donneeMiseAJour = {
      ...ancienneDonnee,
      ...nouvelleDonnee
    };
    fs.writeFileSync(cheminFichier, JSON.stringify(donneeMiseAJour, null, 2), "utf-8");
    return donneeMiseAJour;
  } catch (erreur) {
    console.error("Erreur lors de la mise à jour des données :", erreur);
  }
};

// Gestion des mentions et des filtres
try {
  const { mention, filter } = require(lib_dir + "/asta.js");

  smd({
    cmdname: "mention",
    fromMe: true,
    category: "chats",
    desc: "Configurer une réponse automatique pour les mentions",
    use: "[type URL/audio]",
    usage: "Lisez 'mention wiki' pour toutes les informations.",
    filename: __filename
  }, async (message, args) => {
    mention.cmd(message, args);
  });
  ...
  smd({
    ...
    mention.check(message, args);
  });

  smd({
    pattern: "filter",
    category: "chats",
    desc: "Configurer une réponse automatique pour un mot-clé",
    use: "[asta : comment puis-je vous aider ?]",
    usage: "Configure un mot-clé pour lequel le bot répondra automatiquement.",
    fromMe: true,
    filename: __filename
  }, async (message, args) => {
    filter.set(message, args);
  });

  smd({
    pattern: "fstop",
    category: "chats",
    desc: "Arrêter une réponse automatique pour un mot-clé",
    use: "[asta : comment puis-je vous aider ?]",
    usage: "Supprime un mot-clé déjà configuré pour une réponse automatique.",
    fromMe: true,
    filename: __filename
  }, async (message, args) => {
    filter.stop(message, args);
  });

  smd({
    pattern: "flist",
    category: "chats",
    desc: "Lister les mots-clés avec réponse automatique",
    use: "[asta : comment puis-je vous aider ?]",
    usage: "Affiche une liste de tous les mots-clés configurés.",
    fromMe: true,
    filename: __filename
  }, async (message) => {
    filter.list(message);
  ...
  smd({
    ...
    try {
      filter.check(message, args);
    } catch (erreur) {
      console.error("Erreur dans la vérification du filtre :", erreur);
    }
  });

} catch (erreur) {
  if (!global.showUpdate) {
    console.log("\n⚠️===========================⚠️\n\nMISE À JOUR DISPONIBLE\nVeuillez mettre à jour votre bot dès que possible !\n\n⚠️===========================⚠️");
    global.showUpdate = true;
  }
}

// Commande "afk" pour indiquer qu'on est absent
let afk = false;

smd({
  pattern: "afk",
  desc: "Déclarez-vous absent (away from keyboard)",
  category: "chats"
}, async (message, args) => {
  try {
    let afkData = await db.get();
    afk = afkData.afk || { users: {} };

    if (!args) {
      return message.reply(
        `Exemple : Mon propriétaire est Rayan.\nDernière connexion : #lastseen\nPour désactiver, utilisez ${prefix}unAfk.`
      );
    }
    ...
    afkData.afk = { ...afk };
    await db.update(afkData);

    let messageText = `@${message.sender.split("@")[0]} est maintenant en mode AFK.\nRaison : ${args}`;
    await sendWelcome(message, messageText, message, message.sender);

  } catch (erreur) {
    message.error(`Erreur dans la commande AFK : ${erreur}`);
  }
});

// Commande "unafk" pour annuler l'état d'absence
smd({
  pattern: "unafk",
  desc: "Désactiver l'état d'absence",
  category: "chats"
}, async (message) => {
  try {
    let afkData = await db.get();
    ...
    if (!afk[message.sender]) {
      return message.reply("Vous n'êtes pas en mode AFK.");
    }
    ...
    await message.reply("Vous êtes de retour !");
  } catch (erreur) {
    message.error(`Erreur dans la commande UnAFK : ${erreur}`);
  }
});
let txt = {
  "2": "*Hé, je t'ai déjà informé !*\n",
  "3": "*Arrête de spammer !*"
};
...
function getTimeDifference(_0x47a53) {
  ...
  return (_0x353908 ? "Jours " + _0x353908 + ", " : "") + "Heures " + (_0x349796 || 0) + ", Minutes " + (_0x9c628b || 0);
}
  ...
  try {
    ...
    for (let _0x53ceee = 0; _0x53ceee < _0x3acd2a.length; _0x53ceee++) {
      if (afk[_0x3acd2a[_0x53ceee]] && _0x2769f2.sender !== _0x3acd2a[_0x53ceee]) {
        ...
        let _0x5ab59f = ((_0x208a1a ? _0x208a1a : "") + " *@" + _0x3acd2a[_0x53ceee].split("@")[0] + " est actuellement AFK.*\n*Raison:* " + afk[_0x3acd2a[_0x53ceee]].reason.replace("@lastseen", "\n*Dernière connexion:* " + getTimeDifference(afk[_0x3acd2a[_0x53ceee]].lastseen) + "\n")).trim();
        await sendWelcome(_0x2769f2, _0x5ab59f, _0x2769f2, _0x3acd2a[_0x53ceee]);
      }
    }
  } catch (_0x4f282f) {
    console.log("ERREUR DANS AFK MAIN\n", _0x4f282f);
  }
});
...
smd(
  {
    pattern: "alive",
    desc: "Affiche le statut du système avec différents designs.",
    category: "général",
    filename: __filename,
    use: "alive",
    react: "🙋🏽‍♂️",
      ...
      const designs = [
        async () => {
          const imageBuffer = await axios.get(
            "https://i.imgur.com/1YWlaIx.jpeg",
            {
          ...
          if (!quote || quote.status !== 200) {
            return await message.reply("*Échec de la récupération d'une citation.*");
          }

          const quoteText = `\n\n*"${quote.result.body}"*\n_- ${quote.result.author}_`;
          const end = new Date().getTime();
          const pingSeconds = (end - start) / 1000;
          const captionText = `JINHUYK-MD_V2 \n\n*ʀᴀᴛᴇ ᴅᴇ ʀᴇsᴘᴏɴsᴇ:* ${pingSeconds} secondes${quoteText}\n\nJINHUYK-MD_V2`;

          return { image: imageBuffer.data, caption: captionText };
        },
        async () => {
          const imageBuffer = await axios.get(
            "https://i.imgur.com/6ITNAMg.jpeg",
            {
              responseType: "arraybuffer",
            }
          );
          ...
          if (!fact || fact.status !== 200) {
            return await message.reply("*Échec de la récupération d'un fait.*");
          }

          const end = new Date().getTime();
          const pingSeconds = (end - start) / 1000;
          const captionText = `JINHUYK-MD_V2\n\n*ʀᴀᴛᴇ ᴅᴇ ʀᴇsᴘᴏɴsᴇ:* ${pingSeconds} secondes\n\n*Fait:*\n${fact.result.fact}\n\nJINHUYK-MD_V2`;
        }
      ];
    } catch (error) {
      console.log("Erreur dans la commande alive : ", error);
    }
  }
);
return { image: imageBuffer.data, caption: captionText };
      ...
      await message.error(
        error + "\n\nCommande : alive",
        error,
        "*Échec de l'affichage du statut.*"
      );
    }
  }
);
...
async function convertAudioToBlackScreenVideo(_0x528238, _0x32b9b6) {
  try {
    ...
    console.log("Audio converti avec succès en vidéo écran noir !");
    return {
      result: true
    };
  } catch (_0x2a64be) {
    console.log("Une erreur s'est produite :", _0x2a64be);
    return {
      result: false
    };
  }
...
smd({
  pattern: "audiourl",
  alias: ["black"],
  desc: "Récupère l'URL d'un audio et le convertit en vidéo noire",
  category: "convertisseur"
}, async (_0x72926a, _0x4e5da) => {
  try {
    if (!_0x72926a.quoted) {
      return await _0x72926a.reply("_Répondez au message audio !_");
    }
    let _0x460b3f = "";
    if (_0x72926a.quoted.mtype == "audioMessage") {
      ...
      if (_0x555071.result) {
        ...
        throw "Chemin de média invalide";
      }
    }
  } catch (_0x10a3dc) {
    await _0x72926a.error(_0x10a3dc + "\n\nCommande : audiourl", _0x10a3dc, "_ERREUR !_");
  }
});
smd({
  pattern: "bgm",
  desc: "Activer/Désactiver le bgm",
  fromMe: true,
  category: "divers"
  ...
  try {
    ...
    if (_0x27977e === "on" || _0x27977e === "enable" || _0x27977e === "act") {
      ...
      return await _0x4b6857.reply("*Bgm activé avec succès*");
      ...
      return await _0x4b6857.reply("*Bgm désactivé avec succès*");
    } else {
      return await _0x4b6857.send("*_Utilisez on/off pour activer/désactiver les sons bgm_*");
    }
  } catch (_0x3e23ea) {
    await _0x4b6857.error(_0x3e23ea + "\n\nCommande : bgm ", _0x3e23ea);
  }
});
...
smd({
  pattern: "delbgm",
  fromMe: true,
  desc: "Supprimer une chanson du bgm",
  category: "divers"
}, async (_0x17f7be, _0x490490) => {
  try {
    if (!_0x490490) {
      return await _0x17f7be.reply("*Donne-moi le nom de la chanson à supprimer du BGM*");
    }
    ...
    if (_0x342d7e[_0x490490]) {
      ...
      return await _0x17f7be.reply("*Chanson _" + _0x490490 + "_ supprimée du BGM.*");
    } else {
      return await _0x17f7be.reply("*Le nom _'" + _0x490490 + "'_ n'existe pas dans le BGM.*");
    }
  } catch (_0x3a2540) {
    await _0x17f7be.error(_0x3a2540 + "\n\nCommande : delbgm ", _0x3a2540);
  }
});
...
smd({
  ...
  fromMe: true,
  desc: "Obtenir la liste des bgm",
  category: "divers"
}, async _0x27d048 => {
  try {
    let _0x51712a = " *INFORMATIONS SUR LES CHANSONS BGM*\n";
    ...
    console.log("sons: ", _0x100987);
    for (const _0x410a73 in _0x100987) {
      _0x51712a += "*" + _0x410a73 + ":* " + _0x100987[_0x410a73] + " \n";
    }
    return await _0x27d048.reply(_0x51712a === " *INFORMATIONS SUR LES CHANSONS BGM*\n" ? "*_Aucune chanson définie dans le BGM pour l'instant !_*" : _0x51712a);
  } catch (_0x37e63f) {
    await _0x27d048.error(_0x37e63f + "\n\nCommande : allbgm", _0x37e63f);
  }
});
smd({
  pattern: "addbgm",
  alias: ["abgm", "newbgm"],
  fromMe: true,
  desc: "Ajouter une chanson dans le bgm",
  category: "divers"
}, async (_0x65bfc2, _0x324d8f) => {
  try {
    if (!_0x65bfc2.quoted) {
      return await _0x65bfc2.reply("Uhh, Veuillez répondre à un audio/vidéo pour l'ajouter au BGM");
    }
    if (!_0x324d8f) {
      return await _0x65bfc2.reply("Uhh, Veuillez donner le nom de la chanson du BGM");
    }
    let _0x5e225e = false;
    let _0x44e871 = "";
    ...
    if (!_0x44e871) {
      return await _0x65bfc2.reply("Il y a eu une erreur lors de l'ajout de la chanson BGM");
    }
    ...
    try {
      ...
      return await _0x65bfc2.reply("*Nouvelle chanson ajoutée au BGM avec le nom : " + _0x324d8f + "*");
    } catch (_0x3490af) {
      return await _0x65bfc2.error(_0x3490af);
    }
  } catch (_0xa9b37e) {
    await _0x65bfc2.error(_0xa9b37e + "\n\nCommande : addbgm", _0xa9b37e);
  }
});
...
smd({
  ...
  alias: ["permit"],
  fromMe: true,
  desc: "Activer/Désactiver l'autorisation PM",
  ...
  try {
    ...
    if (!_0x55bc30) {
      return await _0x20ae95.send("*Autorisation PM actuellement *" + (_0x457e3e.permit ? "activée" : "désactivée") + "!!!*\n  *Définie sur:* ```" + _0x457e3e.values.toUpperCase() + "```\n  \n  *Commandes disponibles:*```\n  " + (prefix + _0x4ec31d) + " off \n  " + (prefix + _0x4ec31d) + " on | all\n  " + (prefix + _0x4ec31d) + " on | 212,91``` \n  \n\n" + Config.caption);
    }
    ...
    if (_0x381f23.startsWith("on") || _0x381f23.startsWith("enable") || _0x381f23.startsWith("act")) {
      if (_0x457e3e.permit && _0x457e3e.permit_values === _0x133f61) {
        return await _0x20ae95.send("*_Uhh, l'autorisation PM est déjà activée !_*");
      }
      ...
      return await _0x20ae95.send("*_Autorisation PM " + (_0x110a99 ? "mise à jour" : "activée") + " avec succès !_*\n*_Maintenant " + (_0x133f61 === "all" ? "tout le monde" : _0x133f61) + " doit avoir l'autorisation pour envoyer des PM_*");
    } else if (_0x381f23.startsWith("off") || _0x381f23.startsWith("disable") || _0x381f23.startsWith("deact")) {
      if (!_0x457e3e.permit) {
        return await _0x20ae95.send("*_Uhh, l'autorisation PM est déjà désactivée !_*");
      }
      await bot_.updateOne({
        id: "bot_" + _0x20ae95.user
      }, {
      ...
      return await _0x20ae95.send("*_Autorisation PM désactivée avec succès !!!_*");
    } else {
      return await _0x20ae95.bot.sendMessage(_0x20ae95.chat, {
        text: "*Autorisation PM actuellement *" + (_0x457e3e.permit ? "activée" : "désactivée") + "!!!*\n*Fournir une instruction valide, comme on/off pour activer/désactiver l'autorisation PM.*"
      });
    }
  } catch (_0x4b83b9) {
    await _0x20ae95.error(_0x4b83b9 + "\n\nCommande : " + _0x4ec31d + " ", _0x4b83b9);
  }
});
smd({
  pattern: "approve",
  alias: ["a"],
  fromMe: true,
  desc: "Approuve cette personne pour les messages privés",
  category: "user"
}, async _0x3a6928 => {
  try {
    let _0x5779ad = await bot_.findOne({ id: "bot_" + _0x3a6928.user }) || await bot_.new({ id: "bot_" + _0x3a6928.user });
    if (!_0x5779ad.permit) return _0x3a6928.sendMessage(_0x3a6928.chat, { text: "*Pmpermit désactivé, veuillez l'activer !!*" });
    if (!_0x3a6928.quoted) return _0x3a6928.reply("*Veuillez répondre à un utilisateur pour l'action.*");

    let _0x161de1 = await userdb.findOne({ id: _0x3a6928.quoted.sender }) || await userdb.new({ id: _0x3a6928.quoted.sender });
    if (_0x161de1.permit === "true") return _0x3a6928.reply("*_" + (_0x161de1.name || "utilisateur") + " a déjà la permission pour pm._*");

    await userdb.updateOne({ id: _0x3a6928.quoted.sender }, { permit: "true", times: 0 });
    return _0x3a6928.send("*_Permission accordée à " + (_0x161de1.name || "utilisateur") + " pour pm._*");
  } catch (error) {
    return _0x3a6928.error(error + "\nCommand: approve", error);
  }
});
...
smd({
  ...
  alias: ["da"],
  fromMe: true,
  desc: "Désapprouve l'utilisateur pour pm.",
  category: "user"
}, async _0x16dec1 => {
  try {
    let _0x1762a5 = await bot_.findOne({ id: "bot_" + _0x16dec1.user }) || await bot_.new({ id: "bot_" + _0x16dec1.user });
    if (!_0x1762a5.permit) return _0x16dec1.sendMessage(_0x16dec1.chat, { text: "*Pmpermit désactivé, veuillez l'activer !!*" });
    if (!_0x16dec1.quoted) return _0x16dec1.send("*Veuillez répondre à un utilisateur pour l'action.*");

    let _0x436303 = await userdb.findOne({ id: _0x16dec1.quoted.sender }) || await userdb.new({ id: _0x16dec1.quoted.sender });
    await userdb.updateOne({ id: _0x16dec1.quoted.sender }, { permit: "false" });
    return _0x16dec1.send("*_Permission révoquée pour " + (_0x436303.name || "utilisateur") + " pour pm._*");
  } catch (error) {
    await _0x16dec1.error(error + "\nCommand: disapprove", error);
  }
});
...
smd({
  ...
  if (_0x1eab99.reaction) return;
  try {
    let _0x491548 = await bot_.findOne({ id: "bot_" + _0x1eab99.user }) || await bot_.new({ id: "bot_" + _0x1eab99.user });
      ...
      for (const _0x584d42 in _0x491548.bgmarray) {
        if ((" " + _0x5d1643 + " ").toLowerCase().includes(_0x584d42 + " ")) {
          await _0x1eab99.sendMessage(_0x1eab99.from, {
            audio: { url: _0x491548.bgmarray[_0x584d42] },
            mimetype: "audio/mpeg",
            ptt: true
          });
        }
    ...
    if (_0x1eab99.isCreator || _0x1eab99.isGroup || _0x1eab99.fromMe || _0x1eab99.reaction) return;

    let _0x2e2c67 = await userdb.findOne({ id: _0x1eab99.sender }) || await userdb.new({ id: _0x1eab99.sender, name: _0x1eab99.senderName });
    const _0x5e8c1f = _0x491548.permit_values.split(",");
    const _0x2408ed = _0x491548.permit_values.includes("all") || _0x5e8c1f.some(num => _0x1eab99.sender.toString().startsWith(num));

    if (_0x2408ed && _0x491548.permit && _0x2e2c67.permit === "false") {
      let _0x5f4b92 = _0x2e2c67.times <= 0 ? "*Veuillez patienter jusqu'à ce que mon propriétaire vous réponde.*" : "*Veuillez ne pas spammer, vous avez " + (_0x2e2c67.times + 1) + " avertissements.*";
      await _0x1eab99.reply(_0x5f4b92);
      await userdb.updateOne({ id: _0x1eab99.sender }, { times: _0x2e2c67.times + 1 });

      if (_0x2e2c67.times + 1 >= global.warncount) {
...
pattern: "autobio",
  alias: ["abio"],
  desc: "Active/Désactive la bio automatique",
  fromMe: true,
  category: "général",
  use: "<on/off>"
  ...
  try {
    ...
    if (!_0x1f0ea7) {
      return await _0x50364b.send("*_Bio automatique actuellement désactivée_*" + (bio.autobio == "false" ? "Désactivée_*\n\nUtilise *" + prefix + "autobio on* pour l'activer !" : "Activée_*\n*Paramètre actuel :* " + (bio.autobio == "true" || bio.autobio == "on" ? "@line, ⏰Heure: @time 🚀@bot" : bio.autobio) + "\n\nUtilise *" + prefix + "autobio off* pour la désactiver!") + "\n  \n  Met à jour aussi le statut : *" + prefix + "autobio @bot(botName) @time @date @line(pickupline) @quote*\n  ");
    }
    ...
    if (_0x3d9e60 === "off" || _0x3d9e60 === "disable" || _0x3d9e60 === "deact") {
      ...
      if (bio.autobio === "false") {
        return await _0x50364b.reply("*Bio automatique déjà désactivée*");
      }
      await bot_.updateOne({
        id: "bot_" + _0x50364b.user
      }, {
      ...
      return await _0x50364b.reply("*Bio automatique désactivée avec succès*");
    } else {
      await bot_.updateOne({
        id: "bot_" + _0x50364b.user
      }, {
      ...
      var _0x347b23 = await getContent(_0x50364b, _0x1f0ea7 == "true" || _0x1f0ea7 == "on" ? "Bio automatique par KERM_MD-V2, ⏰Heure: @time 🚀@bot" : _0x1f0ea7);
      await _0x50364b.bot.updateProfileStatus(_0x347b23);
      return await _0x50364b.reply("*Bio automatique activée avec succès*" + ("\n  *Bio définie :* " + _0x347b23 + "  \n  \n  *La bio WhatsApp se met à jour automatiquement toutes les minutes !*\n  "));
    }
  } catch (_0x4c629c) {
    await _0x50364b.error(_0x4c629c + "\n\nCommande : autobio ", _0x4c629c);
  }
  bio = false;
});
  ...
  if (bio && bio.autobio && typeof bio.autobio === "string" && bio.autobio != "false") {
    if (!isStartAutoBio) {
      ...
      abioJob = cron.schedule("*/1.5 * * * *", async () => {
        try {
          var _0x4b4b3b = "`";
          let _0x34c0a6 = bio.autobio == "true" || bio.autobio == "on" ? "Bio automatique par JINHUYK-MD_V2, ⏰Heure: @time 🚀@bot" : bio.autobio;
...
events.cmd({
  cmdname: "logout",
  desc: "Déconnecte le bot en cours d'exécution avec le périphérique !",
  fromMe: true,
  type: "tools"
}, async (_0x2ff7b4, _0x39eee0) => {
  try {
    _0x2ff7b4.bot.logout();
    _0x2ff7b4.reply("DÉCONNECTÉ !");
  } catch (_0x17c62) {
    _0x2ff7b4.reply("_ERREUR !_");
