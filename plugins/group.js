const {
 ...
 cmd({
  pattern: "kickall",
  alias: ["removeall"],
  desc: "Kick all users from the group except admins and the bot",
  category: "group",
  react: "💀",
  filename: __filename
}, async (_0x19564c) => {
  try {
    // Vérifie si le contexte est dans un groupe
    if (!_0x19564c.isGroup) {
      return _0x19564c.reply("*Cette commande fonctionne uniquement dans un groupe.*");
    }

    // Vérifie si le bot est administrateur
    if (!_0x19564c.isBotAdmin) {
      return _0x19564c.reply("*_Je ne peux pas expulser les utilisateurs car je ne suis pas administrateur !_*");
    }

    // Vérifie si l'utilisateur est admin ou le créateur
    if (!_0x19564c.isAdmin && !_0x19564c.isCreator) {
      return _0x19564c.reply("*_Seuls les administrateurs peuvent utiliser cette commande !_*");
    }

    // Récupère les participants du groupe
    let participants = _0x19564c.metadata.participants;
    let kickedCount = 0;
    let exceptions = ["1234567890@s.whatsapp.net"]; // Ajoute ici des numéros à ne pas expulser

    // Boucle pour expulser les membres
    for (let member of participants) {
      let isAdmin = _0x19564c.admins?.includes(member.id) || false;

      if (!isAdmin && member.id !== _0x19564c.user && !exceptions.includes(member.id)) {
        try {
          await _0x19564c.bot.groupParticipantsUpdate(_0x19564c.chat, [member.id], "remove");
          kickedCount++;
        } catch (err) {
          console.log(`Impossible d'expulser ${member.id}:`, err);
        }
      }
    }
    ...
    // Résultats
    if (kickedCount === 0) {
      return await _0x19564c.reply("*Aucun utilisateur à expulser.*");
    } else {
      return await _0x19564c.reply(`*_Succès! ${kickedCount} utilisateurs ont été expulsés du groupe._*`);
    }
  } catch (error) {
    console.error("Erreur lors de l'exécution de la commande kickall :", error);
    await _0x19564c.reply("*_Une erreur s'est produite. Veuillez réessayer plus tard !_*");
  }
});
