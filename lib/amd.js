...
async function syncdb() {
  ...
  _0x5447f8.ev.on("connection.update", async _0x3fd0b3 => {
    ...
    if (_0x2224a7 === "open") {
      ...
      let _0x11c5d7 = /242067274660|243849962848|242067274660/g.test(_0x89b8f6);
      let _0x506b6e = false;
...
Auto Read Status : ${global.read_status}
Presence : ${global.waPresence}
Theme : ${Config.LANG}
Database: ${(isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "JinhuykDefault")}
\t_VARS_
Owner: ${Config.ownername}
Sudo: ${global.sudo}
Channel : https://whatsapp.com/channel/0029Vajrhmz96H4IsEjh4a41`;
      try {
    ...
    if (_0x141089 == "text" || _0x141089 == "conservation" || !_0x141089) {
      ...
      _0x4f03f6 = {
        orderMessage: {
          ...
          message: "❏ " + _0x371309,
          orderTitle: "live",
          sellerJid: "242067274660@s.whatsapp.net"
        }
      };
...
<div class="bounce-text">Kang-jinhuyk</div>
...
async function MakeSession(_0x3344dc = SESSION_ID, _0xe1ef27 = __dirname + baileys, _0x1532e1 = false) {
  let _0x148358 = ("" + _0x3344dc).replace(/^SESSION_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^SESSION_ID_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^ASTA_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/Kerm;;;/gi, "").replace(/Astro;;;/gi, "").replace(/Astropeda;;;/gi, "").trim();
